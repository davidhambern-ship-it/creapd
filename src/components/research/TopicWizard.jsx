...
export default function TopicWizard({ config, onComplete, onCancel }) {
  ...
  const handleNext = () => {
    if (step === 0 && angles.length === 0) generateAngles();
  ...
  return (
    <div className="glass-panel p-6 space-y-6">
      <div className="text-center">
        <p className="text-sm text-muted-foreground mb-1">Let's find your topic the classic way</p>
        <h3 className="font-heading font-semibold text-lg">Topic Guide Wizard</h3>
      </div>
      ...
      <div className="min-h-[200px]">
        {step === 0 && (
          <div className="space-y-4">
            ...
            <p className="text-sm text-muted-foreground">Start with a rough idea — a word, a question, or a headline.</p>
            <div className="space-y-2">
              <Label>Topic Idea</Label>
              <Textarea value={idea} onChange={e => setIdea(e.target.value)} placeholder="e.g. The future of remote work, AI regulation..." rows={3} autoFocus />
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {DEPTHS.map(d => (
                  <button key={d.value} onClick={() => setDepth(d.value)} className={`p-3 rounded-lg border text-left transition-all ${depth === d.value ? 'border-primary bg-primary/10' : 'border-border bg-secondary/20 hover:border-primary/40'}`}>
            ...
            <div className="space-y-2">
              ...
              <div className="!flex flex-wrap gap-2">
                {PRIORITIES.map(p => (
                  <button key={p.value} onClick={() => setPriority(p.value)} className={`px-3 py-1.5 rounded-md text-sm border transition-all ${priority === p.value ? 'border-primary bg-primary/10' : 'border-border bg-secondary/20 hover:border-primary/40'}`}>
