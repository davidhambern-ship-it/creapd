...
import { Activity, Clock, Zap, AlertCircle, TrendingUp, Database, ArrowLeft } from 'lucide-react';
import { ENGINE_STATUS_OPTIONS, OPERATING_MODES } from '@/lib/caeConstants';

export default function CAEEngineHeader({ config }) {
  const statusOpt = config ? (ENGINE_STATUS_OPTIONS.find(s => s.value === config.engine_status) || ENGINE_STATUS_OPTIONS[0]) : ENGINE_STATUS_OPTIONS[0];
  const modeOpt = config ? (OPERATING_MODES.find(m => m.value === config.operating_mode) || OPERATING_MODES[0]) : OPERATING_MODES[0];
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-0.5">
                <Link to="/spiritual/library" className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors">
              ...
              <div className="flex items-center gap-3 text-sm text-muted-foreground mt-0.5">
                ...
                <span className="text-xs text-muted-foreground">· Runs automatically every {config?.scan_interval_minutes || 30} min</span>
              </div>
            </div>
          </div>
        </div>
