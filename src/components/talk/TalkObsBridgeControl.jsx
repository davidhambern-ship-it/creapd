...
function TalkObsBridgeControlLive() {
  ...
  const takeScene = async () => {
    ...
    try {
      ...
      setError(err?.data?.diagnostic?.message || err?.data?.error || err?.message || 'CREAPD could not queue the OBS scene change.');
    } finally {
      setBusy('');
    }
  };
          ...
          <div className="sticky top-0 bg-[#090b10]/97 border-b border-white/10 px-4 py-3 flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground">Bridge setup, diagnostics, and manual fallback</p>
              </div>
            </div>
            ...
            {bridge?.connected ? (
              ...
              <>
                <div className="rounded-xl border border-primary/20 bg-primary/[0.06] p-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">The local bridge talks to OBS on this computer, then checks in to CREAPD over normal HTTPS.</p>
                </div>
                ...
                {bridge?.online && !bridge?.connected && (
                  <div className="rounded-xl border border-amber-500/20 bg-amber-500/[0.07] p-3 flex gap-2">
                    ...
                    <div className="text-xs"><p className="text-amber-200 font-medium">The CREAPD bridge is running, but OBS is not connected.</p><p className="text-muted-foreground mt-1">{bridge.last_error || 'Check OBS WebSocket Server and password.'}</p></div>
                  </div>
                )}
              </>
            )}
