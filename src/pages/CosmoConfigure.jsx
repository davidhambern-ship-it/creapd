...
import {
  SHOW_FORMAT_OPTIONS, TOPIC_OPTIONS, TONE_OPTIONS, RESEARCH_SOURCE_OPTIONS,
  AI_AUTOMATION_OPTIONS, DEFAULT_AI_AUTOMATION, RUNTIME_DEFAULTS
} from '@/lib/cosmoConstants';
import {
  ChevronLeft, ChevronRight, Loader2, Sparkles, Calendar, Clock,
  Smile, ListChecks, Search, Users, Bot, CheckCircle2, Building2
} from 'lucide-react';

const STEPS = [
  { label: 'Show Details', icon: Calendar },
  { label: 'Show Format', icon: Sparkles },
...
export default function CosmoConfigure() {
  ...
  useEffect(() => {
    if (editConfigId) {
      base44.entities.CosmoProductionConfiguration.get(editConfigId).then(c => {
  ...
  const handleBuild = async () => {
    ...
    try {
      let savedConfig;
      if (editConfigId) {
        savedConfig = await base44.entities.CosmoProductionConfiguration.update(editConfigId, config);
      } else {
        savedConfig = await base44.entities.CosmoProductionConfiguration.create(config);
      }

      await base44.auth.updateMe({
        default_production_type: 'cosmo',
        default_production_config_id: savedConfig.id
      });
      await base44.entities.CosmoProductionConfiguration.update(savedConfig.id, { is_default: true });
  ...
  const renderStep = () => {
    switch (step) {
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input value={config.production_name} onChange={e => updateConfig('production_name', e.target.value)} placeholder="The Beauty Lab" />
              </div>
              <div className="space-y-2">
        ...
        return (
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label>Beauty / Demo Runtime (min)</Label>
                <Input type="number" value={config.cosmo_segment_runtime} onChange={e => updateConfig('cosmo_segment_runtime', Number(e.target.value))} />
              </div>
              <div className="space-y-2">
      ...
      case 5:
        return (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">Enter guest expert information (one guest per section, separated by blank lines). Include name, specialty, and any relevant background.</p>
            <Textarea
              value={config.guest_details}
              onChange={e => updateConfig('guest_details', e.target.value)}
              placeholder={"Dr. Sarah Lee — Dermatologist\nBoard-certified, specializes in acne treatment and anti-aging\n\nMia Chen — Celebrity Makeup Artist\n15 years experience, worked with major fashion brands"}
              rows={8}
            />
          </div>
        );
          ...
          <div className="space-y-6">
            ...
            <div className="glass-panel p-5">
              ...
              <div className="grid grid-cols-3 gap-2 text-sm">
                ...
                <div><span className="text-muted-foreground">Beauty:</span> {config.cosmo_segment_runtime} min</div>
