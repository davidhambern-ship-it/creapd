...
import { formatRuntime, SEGMENT_TYPE_LABELS } from '@/lib/sportsConstants';
import { Loader2, Trophy, ClipboardList, AlertCircle, Clock } from 'lucide-react';

export default function SportsRundown() {
  const { config, segments, loading } = useSportsProduction();
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      <div>
        <h1 className="text-2xl font-heading font-bold flex items-center gap-2"><ClipboardList className="w-5 h-5 text-primary" /> Show Rundown</h1>
        <p className="text-sm text-muted-foreground mt-1">Structured show timeline with game previews, analysis, and timings</p>
      </div>
      ...
      {segments.length === 0 ? (
        ...
        <>
          <div className="glass-panel p-4 flex items-center gap-4 text-sm">
            ...
            <span className="text-muted-foreground">Total Segments:</span><span className="font-medium">{segments.length}</span>
            <span className="text-muted-foreground ml-4">Total Runtime:</span><span className="font-medium">{formatRuntime(totalSeconds)}</span>
          </div>
          ...
          <div className="glass-panel p-4 space-y-1">
            {segments.map((item, i) => (
              <div key={item.id} className="flex items-center gap-3 py-2 px-2 rounded hover:bg-white/5 border-b border-white/[0.03] last:border-0">
                ...
                <span className={`text-xs px-2 py-0.5 rounded shrink-0 ${SEGMENT_TYPE_LABELS[item.segment_type] ? 'bg-primary/15 text-primary' : 'bg-muted text-muted-foreground'}`}>{SEGMENT_TYPE_LABELS[item.segment_type] || item.segment_type}</span>
