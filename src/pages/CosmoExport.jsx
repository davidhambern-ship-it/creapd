...
import { Loader2, Sparkles, Download, Package, CheckCircle2 } from 'lucide-react';

export default function CosmoExport() {
  const { config, topics, segments, assets, research, guests, loading } = useCosmoProduction();
  ...
  const handleExport = () => {
    ...
    setTimeout(() => {
      const exportData = {
        ...
        topics, research, guests, segments, assets,
  ...
  const exportItems = [
    ...
    { label: 'Topics', count: topics.length, done: topics.length > 0 },
    { label: 'Guests', count: guests.length, done: guests.length > 0 },
