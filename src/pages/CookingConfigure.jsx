...
import {
  SHOW_FORMAT_OPTIONS, CUISINE_OPTIONS, TONE_OPTIONS, RESEARCH_SOURCE_OPTIONS,
  AI_AUTOMATION_OPTIONS, DEFAULT_AI_AUTOMATION, RUNTIME_DEFAULTS
} from '@/lib/cookingConstants';
import {
  ChevronLeft, ChevronRight, Loader2, ChefHat, Calendar, Clock,
  Smile, ListChecks, Search, Users, Bot, CheckCircle2, Building2
} from 'lucide-react';

const STEPS = [
  { label: 'Show Details', icon: Calendar },
  { label: 'Show Format', icon: ChefHat },
  { label: 'Runtime', icon: Clock },
  { label: 'Cuisines', icon: ListChecks },
...
export default function CookingConfigure() {
  ...
  useEffect(() => {
    if (editConfigId) {
      base44.entities.CookingProductionConfiguration.get(editConfigId).then(c => {
  ...
  const handleBuild = async () => {
    ...
    try {
      let savedConfig;
      if (editConfigId) {
        savedConfig = await base44.entities.CookingProductionConfiguration.update(editConfigId, config);
      } else {
        savedConfig = await base44.entities.CookingProductionConfiguration.create(config);
      }

      await base44.auth.updateMe({
        default_production_type: 'cooking',
        default_production_config_id: savedConfig.id
      });
      await base44.entities.CookingProductionConfiguration.update(savedConfig.id, { is_default: true });

      await base44.functions.invoke('buildCookingProduction', { configuration_id: savedConfig.id });

      navigate('/cooking/dashboard');
    } catch (err) {
  ...
  const renderStep = () => {
    switch (step) {
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input value={config.production_name} onChange={e => updateConfig('production_name', e.target.value)} placeholder="The Kitchen Show" />
              </div>
              <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Textarea value={config.show_description} onChange={e => updateConfig('show_description', e.target.value)} placeholder="Describe your cooking show..." rows={3} />
            </div>
          </div>
        );
      case 1:
        ...
        return (
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label>Cooking / Demo Runtime (min)</Label>
                <Input type="number" value={config.cooking_segment_runtime} onChange={e => updateConfig('cooking_segment_runtime', Number(e.target.value))} />
              </div>
              <div className="space-y-2">
      ...
      case 3:
        return <div className="space-y-3"><p className="text-sm text-muted-foreground mb-4">Choose which cuisines Producer should research and prepare recipes for.</p>{renderTagSelection('cuisines', CUISINE_OPTIONS)}</div>;
      case 4:
      ...
      case 5:
        return (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">Enter guest chef information (one guest per section, separated by blank lines). Include name, specialty, and any relevant background.</p>
            <Textarea
              value={config.guest_details}
              onChange={e => updateConfig('guest_details', e.target.value)}
              placeholder={"Chef Maria Rodriguez — Italian Cuisine\nJames Beard Award winner, specializes in regional Italian cooking\n\nChef Tom Chen — Asian Fusion\nFood Network regular, fusion cuisine expert"}
              rows={8}
            />
          </div>
        );
          ...
          <div className="space-y-6">
            ...
            <div className="glass-panel p-5">
              ...
              <div className="grid grid-cols-3 gap-2 text-sm">
                ...
                <div><span className="text-muted-foreground">Cooking:</span> {config.cooking_segment_runtime} min</div>
            ...
            <div className="glass-panel p-5">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div><span className="text-muted-foreground">Cuisines:</span> {safeParse(config.cuisines, []).length} selected</div>
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-6">
        <div className="max-w-md w-full text-center">
          ...
          <p className="text-muted-foreground mb-8">Producer is generating research, recipes, cooking instructions, rundown, and AI assets. This takes about 30-60 seconds.</p>
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-4xl mx-auto p-6 md:p-10">
        <div className="!flex items-center gap-3 mb-8">
          ...
          <div>
            <h1 className="text-2xl font-heading font-bold">Cooking Production Configuration</h1>
