...
import { Loader2, ChefHat, Download, Package, CheckCircle2 } from 'lucide-react';

export default function CookingExport() {
  const { config, recipes, segments, assets, research, ingredients, loading } = useCookingProduction();
  ...
  const handleExport = () => {
    ...
    setTimeout(() => {
      const exportData = {
        ...
        recipes, research, ingredients, segments, assets,
      ...
      setExporting(false);
      setExportResult({ success: true, count: recipes.length + research.length + ingredients.length + segments.length + assets.length });
    }, 1500);
  };
  ...
  const exportItems = [
    ...
    { label: 'Recipes', count: recipes.length, done: recipes.length > 0 },
    { label: 'Ingredients', count: ingredients.length, done: ingredients.length > 0 },
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Export your complete cooking production package</p>
      </div>
