...
function MetricCard({ label, value, accent, delay }) {
  const accentColor = accent === 'purple' ? 'text-berna-purple'
    : accent === 'emerald' ? 'text-berna-emerald'
    : accent === 'orange' ? 'text-berna-orange'
    : 'text-white';
  return (
    <motion.div
    ...
    >
      ...
      <p className={`text-lg font-heading font-bold ${accentColor}`}>{value}</p>
    </motion.div>
  );
}
    ...
    return (
      <div className="relative flex items-center justify-center h-screen p-6 overflow-hidden">
        ...
        <div className="max-w-md text-center relative z-10">
          ...
          <h2 className="text-xl font-heading font-bold mb-3 neon-underline inline-block">No Music Production Found</h2>
          ...
          <Button asChild size="lg" className="bg-gradient-to-r from-berna-emerald to-berna-purple hover:opacity-90 text-white">
    ...
    return (
      <div className="relative flex items-center justify-center h-screen p-6 overflow-hidden">
        ...
        <div className="max-w-md text-center relative z-10">
          ...
          <h2 className="text-xl font-heading font-bold mb-3 neon-underline inline-block">Building Your Music Production</h2>
          ...
          <div className="space-y-3 text-left">
            {['Generating playlist plan', 'Researching music topics', 'Building show rundown', 'Generating AI assets'].map((label, i) => (
              <div key={i} className="flex items-center gap-3 text-sm glass-panel px-4 py-2.5">
    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <div className="relative z-10 p-6 md:p-8 space-y-6">
        ...
        >
          <div>
            <div className="flex items-center gap-2 mb-1">
              <Music className="w-5 h-5 text-berna-purple" />
              <h1 className="text-2xl font-heading font-bold text-white">{config.production_name}</h1>
            </div>
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={handleRefresh} className="hover:border-berna-purple/50">
              <RefreshCw className="w-4 h-4 mr-1" />
              Refresh
            </Button>
            <Button variant="outline" size="sm" asChild className="hover:border-berna-orange/50">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          <MetricCard label="Total Runtime" value={formatMinutes(config.total_show_runtime)} delay={0.05} />
          <MetricCard label="Music Runtime" value={formatMinutes(config.required_music_runtime)} accent="purple" delay={0.1} />
          <MetricCard label="Genres" value={`${genres.length} selected`} delay={0.15} />
          <MetricCard label="Mood" value={`${moods.length} selected`} delay={0.2} />
          <MetricCard label="Tone" value={config.show_tone} delay={0.25} />
          <MetricCard label="Readiness" value={`${readinessPercent}%`} accent="emerald" delay={0.3} />
        </div>

        {/* Quick Actions */}
        <div className="flex flex-wrap gap-2">
          <Button size="sm" variant="outline" onClick={handleRefresh} className="hover:border-berna-purple/50"><RefreshCw className="w-4 h-4 mr-1" /> Refresh Production</Button>
          <Button size="sm" variant="outline" asChild className="hover:border-berna-orange/50"><Link to="/music/playlist"><ListMusic className="w-4 h-4 mr-1" /> Playlist Builder</Link></Button>
          <Button size="sm" variant="outline" asChild className="hover:border-berna-emerald/50"><Link to="/music/topics"><Mic className="w-4 h-4 mr-1" /> Music Topics</Link></Button>
          <Button size="sm" variant="outline" asChild className="hover:border-berna-purple/50"><Link to="/music/rundown"><ClipboardList className="w-4 h-4 mr-1" /> Show Rundown</Link></Button>
          <Button size="sm" variant="outline" asChild className="hover:border-berna-orange/50"><Link to="/music/assets"><Sparkles className="w-4 h-4 mr-1" /> AI Assets</Link></Button>
          <Button size="sm" variant="outline" asChild className="hover:border-berna-emerald/50"><Link to="/music/export"><Download className="w-4 h-4 mr-1" /> Export</Link></Button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="glass-panel p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-heading font-semibold text-white flex items-center gap-2 neon-underline"><ListMusic className="w-4 h-4 text-berna-purple" /> Playlist Plan</h3>
              <span className={`text-xs px-2 py-1 rounded-full ${playlistStatus === 'Runtime Matched' ? 'bg-berna-emerald/15 text-berna-emerald' : 'bg-berna-orange/15 text-berna-orange'}`}>
                {playlistStatus}
              </span>
            </div>
            {playlist.length > 0 ? (
          ...
          <div className="glass-panel p-5">
            <h3 className="font-heading font-semibold text-white flex items-center gap-2 mb-4 neon-underline"><Clock className="w-4 h-4 text-berna-orange" /> Runtime Status</h3>
          ...
          <div className="glass-panel p-5">
            <h3 className="font-heading font-semibold text-white flex items-center gap-2 mb-4 neon-underline"><Mic className="w-4 h-4 text-berna-emerald" /> Selected Topics</h3>
            {topics.length > 0 ? (
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {topics.map(topic => (
                  <div key={topic.id} className="flex items-center justify-between text-sm py-1.5 px-2 rounded hover:bg-white/5">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${topic.status === 'ready' ? 'bg-berna-emerald/15 text-berna-emerald' : 'bg-muted text-muted-foreground'}`}>
                      {topic.status}
                    </span>
                  </div>
                ))}
          ...
          <div className="glass-panel p-5">
            <h3 className="font-heading font-semibold text-white flex items-center gap-2 mb-4 neon-underline"><TrendingUp className="w-4 h-4 text-berna-purple" /> Research Updates</h3>
            {research.length > 0 ? (
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {research.slice(0, 6).map(item => (
                  <div key={item.id} className="text-sm py-1.5 px-2 rounded hover:bg-white/5">
                    ...
                    <div className="flex items-center gap-2 text-xs text-muted-foreground">
                      ...
                      <span className={`px-1.5 py-0.5 rounded ${item.relevance === 'high' ? 'bg-berna-emerald/15 text-berna-emerald' : 'bg-muted'}`}>{item.relevance}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="glass-panel p-5">
            <h3 className="font-heading font-semibold text-white flex items-center gap-2 mb-4 neon-underline"><ClipboardList className="w-4 h-4 text-berna-orange" /> Show Rundown Preview</h3>
            {rundown.length > 0 ? (
              <div className="space-y-1 max-h-48 overflow-y-auto">
                {rundown.slice(0, 8).map((item, i) => (
                  <div key={item.id} className="flex items-center gap-2 text-sm py-1.5 px-2 rounded hover:bg-white/5">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded ${SEGMENT_TYPE_LABELS[item.segment_type] ? 'bg-berna-purple/15 text-berna-purple' : 'bg-muted'}`}>
          ...
          {/* AI Assets Widget */}
          <div className="glass-panel p-5">
            <h3 className="font-heading font-semibold text-white flex items-center gap-2 mb-4 neon-underline"><Sparkles className="w-4 h-4 text-berna-emerald" /> AI Generated Assets</h3>
            {assets.length > 0 ? (
        ...
        <div className="glass-panel p-5">
          <h3 className="font-heading font-semibold text-white mb-4 neon-underline inline-block">Production Checklist</h3>
...
function RuntimeRow({ label, minutes, highlight }) {
  return (
    <div className="flex items-center justify-between">
      <span className="text-muted-foreground">{label}</span>
      <span className={`font-medium ${highlight ? 'text-berna-purple' : ''}`}>{formatMinutes(minutes)}</span>
    </div>
  );
}
