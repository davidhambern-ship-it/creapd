  ...
  return (
    <div className="min-h-screen bg-[#07090d] text-foreground">
      ...
      <main className="p-4 md:p-6 space-y-4 max-w-[1800px] mx-auto">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1.7fr)_minmax(320px,0.8fr)] gap-4">
          <section className="space-y-4">
            <div className="rounded-2xl border border-white/10 bg-gradient-to-br from-white/[0.06] to-white/[0.02] overflow-hidden">
              <div className="aspect-video bg-black/70 flex items-center justify-center relative">
                ...
                <div className="text-center max-w-md px-6">
                  ...
                  <p className="text-sm text-muted-foreground mt-2">
                    CREAPD Live is running the show state now. OBS / camera transport is intentionally not connected in this first Preview slice.
                  </p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
                <h3 className="font-heading font-semibold flex items-center gap-2 mb-3"><Mic2 className="w-4 h-4 text-primary" /> Host Notes</h3>
                {currentTopic?.talking_points ? (
                  <div className="space-y-4">
                    <div>
                      <p className="text-xs uppercase tracking-wider text-muted-foreground mb-2">{currentTopic.topic_name} talking points</p>
                      <p className="text-sm md:text-base whitespace-pre-line leading-relaxed">{currentTopic.talking_points}</p>
                    </div>
                    {Array.isArray(currentTopic.debate_questions) && currentTopic.debate_questions.length > 0 && (
                      <div className="border-t border-white/10 pt-3">
                        <p className="text-xs uppercase tracking-wider text-muted-foreground mb-2">Questions to push the conversation</p>
                        <ul className="space-y-2 text-sm">
                          {currentTopic.debate_questions.slice(0, 3).map((question, index) => <li key={index}>• {question}</li>)}
                        </ul>
                      </div>
                    )}
                  </div>
                ) : currentSegment?.notes ? (
                  <p className="text-sm md:text-base whitespace-pre-line leading-relaxed">{currentSegment.notes}</p>
                ) : hostScript?.content ? (
                  <p className="text-sm md:text-base whitespace-pre-line leading-relaxed line-clamp-[12]">{hostScript.content}</p>
                ) : (
                  <p className="text-sm text-muted-foreground">No specific host notes were generated for this segment.</p>
                )}
              </div>
          ...
          <aside className="space-y-4">
            <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-4 xl:sticky xl:top-20">
              ...
              <div className="space-y-1 max-h-[56vh] overflow-y-auto pr-1">
                {segments.map((segment, index) => {
                  ...
                  return (
                    <div key={segment.id} className={`rounded-lg px-3 py-2 border ${active ? 'border-primary/40 bg-primary/10' : 'border-transparent bg-white/[0.025]'}`}>
