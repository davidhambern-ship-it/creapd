...
import { formatRuntime, formatMinutes, ASSET_TYPE_LABELS, SEGMENT_TYPE_LABELS } from '@/lib/cookingConstants';
import {
  ChefHat, RefreshCw, Search, Carrot, ClipboardList, Sparkles, Download,
...
export default function CookingDashboard() {
  const { config, recipes, research, ingredients, segments, assets, loading, refresh } = useCookingProduction();
  ...
  useEffect(() => {
    if (config?.status === 'building') {
      const interval = setInterval(async () => {
        if (config?.id) {
          const updated = await base44.entities.CookingProductionConfiguration.get(config.id);
  ...
  const handleRefresh = async () => {
    ...
    setRefreshing(true);
    try {
      await base44.entities.CookingProductionConfiguration.update(config.id, { status: 'building' });
      await base44.functions.invoke('buildCookingProduction', { configuration_id: config.id });
      refresh();
    } catch (err) {
      console.error(err);
    } finally {
    ...
    return (
      <div className="flex items-center justify-center h-screen p-6">
        <div className="max-w-md text-center">
          ...
          <h2 className="text-xl font-heading font-bold mb-3">No Cooking Production Found</h2>
          <p className="text-muted-foreground mb-6">Configure your cooking production to get started. Producer will build everything automatically.</p>
          <Button asChild size="lg"><Link to="/cooking/configure">Configure Production</Link></Button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center h-screen p-6">
        <div className="max-w-md text-center">
          ...
          <h2 className="text-xl font-heading font-bold mb-3">Building Your Cooking Production</h2>
          <p className="text-muted-foreground mb-8">Generating research, recipes, cooking instructions, rundown, and AI assets...</p>
          <div className="space-y-3 text-left">
            {['Researching cuisines', 'Generating recipes', 'Building show rundown', 'Generating AI assets'].map((label, i) => (
  ...
  const checklist = [
    ...
    { label: 'Recipes Generated', done: recipes.length > 0 },
  ...
  return (
    <div className="p-6 md:p-8 space-y-6">
      <div className="!flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="!flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
            ...
            <span className="!flex items-center gap-1"><ChefHat className="w-3.5 h-3.5" /> {config.show_format}</span>
          </div>
        </div>
        <div className="!flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={handleRefresh}><RefreshCw className="w-4 h-4 mr-1" /> Refresh</Button>
          <Button variant="outline" size="sm" asChild>
            <Link to={`/cooking/configure?config_id=${config.id}`}><Settings className="w-4 h-4 mr-1" /> Edit Config</Link>
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        ...
        <div className="glass-panel p-4"><p className="text-xs text-muted-foreground mb-1">Cooking Runtime</p><p className="text-lg font-heading font-bold text-primary">{formatMinutes(config.cooking_segment_runtime)}</p></div>
        ...
        <div className="glass-panel p-4"><p className="text-xs text-muted-foreground mb-1">Ingredients</p><p className="text-sm font-medium">{ingredients.length} listed</p></div>
      ...
      <div className="!flex flex-wrap gap-2">
        ...
        <Button size="sm" variant="outline" asChild><Link to="/cooking/research"><Search className="w-4 h-4 mr-1" /> Research</Link></Button>
        <Button size="sm" variant="outline" asChild><Link to="/cooking/recipes"><ChefHat className="w-4 h-4 mr-1" /> Recipes</Link></Button>
        <Button size="sm" variant="outline" asChild><Link to="/cooking/ingredients"><Carrot className="w-4 h-4 mr-1" /> Ingredients</Link></Button>
        <Button size="sm" variant="outline" asChild><Link to="/cooking/rundown"><ClipboardList className="w-4 h-4 mr-1" /> Show Rundown</Link></Button>
        <Button size="sm" variant="outline" asChild><Link to="/cooking/assets"><Sparkles className="w-4 h-4 mr-1" /> AI Assets</Link></Button>
        <Button size="sm" variant="outline" asChild><Link to="/cooking/export"><Download className="w-4 h-4 mr-1" /> Export</Link></Button>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-panel p-5">
          <div className="!flex items-center justify-between mb-4">
            <h3 className="font-heading font-semibold !flex items-center gap-2"><ChefHat className="w-4 h-4 text-primary" /> Recipes</h3>
            <Link to="/cooking/recipes" className="text-xs text-primary hover:underline">View all</Link>
          </div>
          {recipes.length > 0 ? (
            <div className="space-y-2 max-h-48 overflow-y-auto">
              {recipes.slice(0, 6).map(recipe => (
                <div key={recipe.id} className="!flex items-center justify-between text-sm py-1.5 px-2 rounded hover:bg-white/5">
                  <div className="min-w-0">
                    <p className="font-medium truncate">{recipe.recipe_name}</p>
                    {recipe.difficulty && <p className="text-xs text-muted-foreground capitalize">{recipe.difficulty} · {recipe.prep_time_minutes}min prep · {recipe.cook_time_minutes}min cook</p>}
                  </div>
                  <span className={`text-xs px-2 py-0.5 rounded-full shrink-0 ${recipe.status === 'approved' ? 'bg-emerald-500/15 text-emerald-400' : 'bg-muted text-muted-foreground'}`}>{recipe.status}</span>
                </div>
              ))}
            </div>
          ) : <EmptyState message="No recipes generated yet." actionLabel="Generate" onAction={handleRefresh} />}
        </div>
        ...
        <div className="glass-panel p-5">
          <div className="!flex items-center justify-between mb-4">
            ...
            <Link to="/cooking/research" className="text-xs text-primary hover:underline">View all</Link>
          </div>
          {research.length > 0 ? (
            <div className="space-y-2 max-h-48 overflow-y-auto">
        ...
        <div className="glass-panel p-5">
          <div className="!flex items-center justify-between mb-4">
            ...
            <Link to="/cooking/rundown" className="text-xs text-primary hover:underline">View all</Link>
          </div>
          {segments.length > 0 ? (
            <div className="space-y-1 max-h-48 overflow-y-auto">
              ...
              <Button size="sm" variant="ghost" asChild className="w-full mt-2"><Link to="/cooking/rundown">Open Rundown <ArrowRight className="w-3 h-3 ml-1" /></Link></Button>
            </div>
        ...
        <div className="glass-panel p-5">
          <div className="!flex items-center justify-between mb-4">
            ...
            <Link to="/cooking/assets" className="text-xs text-primary hover:underline">View all</Link>
          </div>
          {assets.length > 0 ? (
            <div className="grid grid-cols-2 gap-2">
              ...
              <Button size="sm" variant="ghost" asChild className="col-span-2 mt-1"><Link to="/cooking/assets">View All Assets <ArrowRight className="w-3 h-3 ml-1" /></Link></Button>
            </div>
