...
Deno.serve(async (req) => {
  try {
    ...
    // News-dedicated asset types
    const NEWS_ASSETS = ['teleprompter_script', 'story_summary', 'talking_points', 'lower_third_text', 'headline_suggestions', 'image_prompt', 'thumbnail_prompt', 'visual_suggestions', 'broll_suggestions', 'social_caption', 'fact_check_notes'];
    const NEWS_ASSET_DESCRIPTIONS = {
    ...
    // Optionally override asset types from ContentDomain config
    const domainKey = content_domain || 'news';
    let ALL_ASSETS = NEWS_ASSETS;
    let ASSET_DESCRIPTIONS = NEWS_ASSET_DESCRIPTIONS;
    if (content_domain && content_domain !== 'news') {
      ...
      if (domain && domain.asset_types && domain.asset_description) {
        try {
          ALL_ASSETS = JSON.parse(domain.asset_types);
          ASSET_DESCRIPTIONS = JSON.parse(domain.asset_description);
        } catch {}
      }
    }
    ...
    if (custom_prompt && custom_prompt.trim()) {
      ...
      prompt = `You are a professional broadcast news producer. Generate production assets for the following news story.
...
Return a JSON object with these exact string keys: ${[...requestedAssets, 'estimated_runtime'].join(', ')}. Each value should be a string with the generated content.`;
    ...
    const llmOptions = { prompt, response_json_schema: responseSchema };
    ...
    // Save to ProductionPackage — always news profile
    ...
    if (existing && existing.length > 0) {
      ...
      if (prompt_template_id) updateFields.prompt_template_id = prompt_template_id;
      if (custom_prompt) updateFields.custom_prompt = custom_prompt;
      pkg = await base44.entities.ProductionPackage.create({ article_id, ...updateFields });
    }
