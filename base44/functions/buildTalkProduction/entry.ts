...
Deno.serve(async (req) => {
  try {
    ...
    const config = await base44.entities.TalkProductionConfiguration.get(configuration_id);
    if (!config) {
      return Response.json({ error: 'Configuration not found' }, { status: 404 });
    }

    await base44.entities.TalkProductionConfiguration.update(configuration_id, { status: 'building' });
    ...
    await base44.entities.TalkTopic.deleteMany({ configuration_id });
    await base44.entities.TalkResearchItem.deleteMany({ configuration_id });
    await base44.entities.TalkGuest.deleteMany({ configuration_id });
    await base44.entities.TalkSegment.deleteMany({ configuration_id });
    await base44.entities.TalkAsset.deleteMany({ configuration_id });

    // ===== LLM CALL 1: Research + Topics (with web search) =====
    const prompt1 = `You are a professional talk show production assistant. You help produce talk shows, interview programs, panel discussions, and conversation-driven presentations.

SHOW DETAILS:
- Production Name: ${config.production_name || 'Talk Show'}
...
- Show Format: ${config.show_format || 'Interview Show'}
- Show Tone: ${config.show_tone || 'Conversational'}
- Show Description: ${config.show_description || 'N/A'}

SELECTED TOPICS:
${topics.join(', ') || 'None selected — choose relevant current topics'}

RESEARCH SOURCES (use these for research):
${researchSources.join(', ') || 'All available sources'}

GUEST DETAILS (if provided):
${config.guest_details || 'No specific guests provided — suggest relevant experts/guests if appropriate'}
...
2. TOPICS: For each selected topic, generate: a summary of current developments, 3-5 talking points (as a single string with line breaks), sources consulted, and suggested_placement in the show (e.g. "Opening Segment", "Deep Dive", "Closing Discussion").
...
Return a JSON object with exactly these keys: research (array), topics (array).`;
    ...
    if (researchData.length > 0) {
      await base44.entities.TalkResearchItem.bulkCreate(researchData);
    }
    ...
    if (topicData.length > 0) {
      await base44.entities.TalkTopic.bulkCreate(topicData);
    }
    ...
    const prompt2 = `You are a professional talk show production assistant. Based on the following topics, research, and show configuration, generate the show rundown and AI production assets.
...
- Show Format: ${config.show_format || 'Interview Show'}
- Total Show Runtime: ${config.total_show_runtime || 60} minutes
- Talk/Segment Runtime: ${config.talk_segment_runtime || 45} minutes
- Commercial/Sponsor Runtime: ${config.commercial_sponsor_runtime || 8} minutes
...
1. SHOW RUNDOWN: Create a structured show rundown that fills the total show runtime (${config.total_show_runtime || 60} minutes). Include: intro, host monologue, interviews, panel discussions, talk breaks, topic segments, sponsor breaks, station IDs, transitions, and outro. Each rundown item must include: order, segment_type (one of: intro, host_monologue, interview, panel_discussion, debate, solo_commentary, audience_qa, sponsor_break, station_id, transition, outro), title, duration_seconds, start_time (HH:MM), end_time (HH:MM), and notes.
...
Return a JSON object with exactly these keys: rundown (array), host_intro (string), host_outro (string), host_script (string), cohost_script (string), guest_intro (string), discussion_questions (string), audience_prompts (string), social_captions (array), hashtags (string), thumbnail_prompt (string), presentation_prompt (string), production_notes (string).`;
    ...
    const validSegmentTypes = ['intro', 'host_monologue', 'interview', 'panel_discussion', 'debate', 'solo_commentary', 'audience_qa', 'sponsor_break', 'station_id', 'transition', 'outro'];
    const rundownData = (llm2.rundown || []).map((item, idx) => ({
      configuration_id,
      order: item.order || idx,
      segment_type: validSegmentTypes.includes(item.segment_type) ? item.segment_type : 'solo_commentary',
      title: item.title || '',
    ...
    if (rundownData.length > 0) {
      await base44.entities.TalkSegment.bulkCreate(rundownData);
    }
    ...
    try {
      ...
      const adapterResponse = await base44.functions.invoke('createAPDReadyPackage', {
        production_profile: 'talk',
        source_entity_type: 'TalkProductionConfiguration',
        source_entity_id: configuration_id,
        configuration_id,
        title: config.production_name || 'Talk Show',
        headline: config.production_name || 'Talk Show',
