...
1. MESSAGE SECTIONS: Create a structured message outline with the following sections (adapt to production type):
   ...
   Each section is a PRESENTATION SCENE. Each must include: order, section_type (from the list above), title, content (the full narration text for this scene — the spoken script that will be converted to voice), speaker_notes (private notes for the speaker), scripture_references (with citations), citations (source citations), estimated_duration_seconds (realistic speaking time based on word count at 130 wpm), target_runtime_seconds (intended duration for this scene, proportional to total target runtime), slide_title (concise title for the presentation slide for this scene), slide_content (bullet points or key phrases for the slide — NOT the full script), slide_visual_prompt (a DETAILED AI image generation prompt describing the visual scene — include subject, style, mood, colors, composition, and lighting. Example: "A serene mountain landscape at sunrise with golden light streaming through clouds, contemplative and majestic, oil painting style, warm earth tones, wide-angle composition"), transition (transition effect when advancing to this slide: one of "fade", "slide_left", "zoom", "dissolve", "none"), and scene_data (a JSON STRING object — the "Choreography JSON" — that directs the entire motion design for this slide. It MUST follow this exact structure: { "motion": { "entrance": one of "fade"|"zoom"|"slide_left"|"dissolve" (the overall slide entrance, should match the transition field), "intensity": one of "calm"|"energetic"|"dramatic" }, "scripture_popups": array of { reference, text, delay (seconds from slide start when the popup should appear) } for scripture verses that should appear as animated popups, "quotes": array of { text, attribution, delay (seconds) } for inspirational or scholarly quotes that should animate in during the narration, "graphics": array of strings describing supporting graphics needed (e.g. "chart: spiritual growth over time", "map: ancient Near East"), "element_choreography": array of { type: one of "title"|"bullet"|"scripture_popup"|"quote"|"graphic", index: number (0-based index into the matching element group), animation: one of "fade-up"|"fade-in"|"zoom-in"|"pop"|"slide-right"|"slide-left", delay: number (seconds from slide start), duration: number (seconds for the animation to complete) } — this array defines the EXACT sequence and timing of every animated element on the slide, synchronized to the narration content and estimated_duration_seconds. You MUST analyze the narration content and slide_content to determine which animations fit where: use "zoom-in" or "pop" for emphasis on key terms, "fade-up" for sequential bullet points, "slide-right" for quotes, and time delays so elements appear as the narrator reaches that part of the script. Every title and every bullet point MUST have a choreography entry. Delays should be spread across the estimated_duration_seconds so the slide feels alive throughout the narration).
